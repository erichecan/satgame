...
type Option = { t: string; correct?: boolean; trap?: string };
type ReadItem = {
  id: string;
  payload: {
    sentences: string[];
...
type Phase = "answer" | "evidence" | "done";
...
export default function ReadTheGreenPage() {
  ...
  useEffect(() => {
    fetch("/api/game-items?gameType=read_the_green")
      .then((r) => r.json())
  ...
  function next() {
    if (idx === items.length - 1) {
      setIdx(items.length); // move past last -> summary
      return;
    }
    setIdx((i) => i + 1);
    resetRound();
  ...
  return (
    <main className="mx-auto w-full max-w-lg flex-1 px-4 py-8">
      ...
      <p className="mt-1 text-sm text-slate-500">先回答问题，再点出证明答案的那句话。</p>
